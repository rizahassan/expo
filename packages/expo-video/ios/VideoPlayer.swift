...
internal final class VideoPlayer: SharedRef<AVPlayer>, Hashable, VideoPlayerObserverDelegate {
  ...
  var playbackRate: Float = 1.0 {
    didSet {
      if oldValue != playbackRate {
        sendEventOnJSThread(name: "playbackRateChange", args: oldValue, playbackRate)
      }
  ...
  var volume: Float = 1.0 {
    didSet {
      if oldValue != volume {
        ...
        sendEventOnJSThread(name: "volumeChange", args: oldVolumeEvent, newVolumeEvent)
      }
  ...
  var isMuted: Bool = false {
    didSet {
      if oldValue != isMuted {
        ...
        sendEventOnJSThread(name: "volumeChange", args: oldVolumeEvent.isMuted, newVolumeEvent.isMuted)
      }
  ...
  func onStatusChanged(player: AVPlayer, oldStatus: PlayerStatus?, newStatus: PlayerStatus, error: Exception?) {
    ...
    sendEventOnJSThread(name: "statusChange", args: oldStatus, newStatus, errorRecord)
    status = newStatus
  }

  func onIsPlayingChanged(player: AVPlayer, oldIsPlaying: Bool?, newIsPlaying: Bool) {
    sendEventOnJSThread(name: "playingChange", args: oldIsPlaying, newIsPlaying)
  ...
  func onPlayedToEnd(player: AVPlayer) {
    sendEventOnJSThread(name: "playToEnd")
    if loop {
  ...
  func onItemChanged(player: AVPlayer, oldVideoPlayerItem: VideoPlayerItem?, newVideoPlayerItem: VideoPlayerItem?) {
    sendEventOnJSThread(name: "sourceChange", args: oldVideoPlayerItem?.videoSource, newVideoPlayerItem?.videoSource)
  }

  // MARK: - Utils

  private func sendEventOnJSThread(name: String, args: AnyArgument...) {
    guard let appContext else {
      return
    }

    appContext.executeOnJavaScriptThread { [weak self] in
      self?.sendEvent(name: name, args: args)
    }
  }
